...
class CalendarStrip extends Component {
  ...
  handleDropdownSelection = () => {
    ...
    const options = dropdownValues.map(({label}) => label);
    if (Platform.OS = 'ios') {
      ActionSheetIOS.showActionSheetWithOptions({
        ...
        const correctValue = dropdownValues[buttonIndex - 1].value;
        this.setState({
          ...
          <View style={styles.headerWrapper}>
            <TouchableOpacity style={styles.headerTouchable} onPress={this.handleDropdownSelection}>
              <Text style={styles.headerDate}>
                {this.state.header} IN {currentDropdownValue}
              </Text>
              <Image source={require('../assets/subtract.png')} style={styles.headerCarret}/>
            </TouchableOpacity>
          </View>
        </View>
    );
